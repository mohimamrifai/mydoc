...
export function DataTable<TData, TValue>({
    ...
    return (
        <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-2 sm:w-1/2 lg:w-1/3 my-2">
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <TableRow key={headerGroup.id}>
                                {headerGroup.headers.map((header) => (
                                    <TableHead 
                                        className={header.column.getIsPinned() ? "sticky right-0 z-10 bg-background shadow-sm" : ""} 
                                        key={header.id}
                                    >
                                        {flexRender(
                            ...
                            table.getRowModel().rows.map((row) => (
                                <TableRow key={row.id}>
                                    {row.getVisibleCells().map((cell) => (
                                        <TableCell 
                                            className={cell.column.getIsPinned() ? "sticky right-0 z-10 bg-background shadow-sm" : ""} 
                                            key={cell.id}
                                        >
                                            {flexRender(
